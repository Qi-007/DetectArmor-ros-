...
{ 
    ...
    m_armors_publish = this->create_publisher<armor_interfaces::msg::Armor>("armor_detector/armors", rclcpp::SensorDataQoS().keep_last(50));

    // 订阅原始图像 " video_frames " 主题  
    m_frame_sub = this->create_subscription<sensor_msgs::msg::Image>(
        "video_frames", 
        rclcpp::SensorDataQoS().keep_last(50), 
        std::bind(&ArmorDetectNode::subFrameCallback, this, std::placeholders::_1)
    );
}

    void ArmorDetectNode::subFrameCallback(const sensor_msgs::msg::Image::ConstSharedPtr img_msg){
