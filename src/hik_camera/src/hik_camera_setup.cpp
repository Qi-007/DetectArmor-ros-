...
void HikDriver::setGain(float gain) {
    // 先获取当前值检查范围
    MVCC_FLOATVALUE current_gain;
    if(check(MV_CC_GetFloatValue(m_handle, "Gain", &current_gain), "getGain")) {
        // 确保gain在有效范围内
        float valid_gain = std::min(std::max(gain, current_gain.fMin), current_gain.fMax);
        check(MV_CC_SetFloatValue(m_handle, "Gain", valid_gain), "setGain");
    }
}
